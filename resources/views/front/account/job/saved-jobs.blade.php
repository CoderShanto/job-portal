...
function removeJob(id) {
    if (confirm("Are you sure you want to remove?")) {
        $('#ajax-message').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> Removing...</div>');
        $.ajax({
            url: '{{ route("account.removeSavedJob") }}',
            type: 'post',
            data: { id: id, _token: '{{ csrf_token() }}' }, // Ensure CSRF token is included
            dataType: 'json',
    ...
    if (response.status) {
        ...
        // Optionally refresh the page after a few seconds or remove the row
        setTimeout(() => {
            location.reload(); // or manually remove the row using jQuery
        }, 1500);
