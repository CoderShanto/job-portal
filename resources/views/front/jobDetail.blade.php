...
function applyJob(id) {
    if (confirm("Are you sure you want to apply on this job?")) {
        // show "Applying..." instantly
        $('.job_details_area').find('#ajax-msg').remove();
        $('.job_details_area').prepend(
            `<div class="alert alert-info" id="ajax-msg">Applying for job...</div>`
        );
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let html = `<div class="alert alert-${alertType}" id="ajax-msg">${response.message}</div>`;

                // show message instantly
                $('.job_details_area').find('#ajax-msg').remove();
                $('.job_details_area').prepend(html);

                // optional reload after 0.5s (faster)
                if (response.status) {
                    setTimeout(function () {
                        location.reload();
                    }, 1500); // was 1500
            ...
            error: function() {
                $('.job_details_area').find('#ajax-msg').remove();
                $('.job_details_area').prepend(
                    `<div class="alert alert-danger" id="ajax-msg">Something went wrong. Please try again.</div>`
                );
            }
        });
    }
